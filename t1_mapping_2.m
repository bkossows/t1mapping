%%%% vfa t1-map
%dependencies: spm, coregister, do_coregister, t1_calc, fillgaps



%%%%%%%%%%%
%%%%INPUTS
%depend on: my_rdir (recursive search)
%%%%%%%%%%%%

clear all; close all;

inputs=my_rdir('**/gre_4*/gre*1.nii');
gre18=cell(size(inputs));
rfmap=cell(size(inputs));
for i=1:size(inputs,1)
    subj=fileparts(fileparts(inputs{i}));
    gre18(i)=my_rdir([subj,'/gre_18*/gre*1.nii']);
    rfmap(i)=my_rdir([subj,'/rf_map*/rfmaps*2.nii']);
end
inputs(:,2)=gre18';
inputs(:,3)=rfmap';
...
for i=1:size(inputs,1)
    ...
    raw_rf_img=[imgs(3).fname(1:end-5),'1.nii'];
    imgs(3)=repair_img(imgs(3).fname); %%%CONSIDER MASKING
    %coregister est&reslice RF-Map
    imgs(3)=coregister(imgs(1).fname,raw_rf_img,imgs(3).fname,1); %ref src oth interp
    % smooth rf map
